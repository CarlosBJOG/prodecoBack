...
(function ( $ ) {
	...
	$.fn.validate_generales_familiar  = function( options ) {
		...
		if($("#nombre_f").val().length==0){ alert("Introduzca el nombre del familiar"); $("#nombre_f").focus(); } else if($("#apellido_p_f").val().length==0){ alert("Introduzca el apellido paterno del familiar"); $("#apellido_p_f").focus(); }else if($("#apellido_m_f").val().length==0){ alert("Introduzca el apellido paterno del familiar"); $("#apellido_m_f").focus(); } else if($("#edad_f").val().length==0) { alert("Introduzca la edad del familiar"); $("#edad_f").focus(); } else if($("#sexo_f").val()=="") { alert("Selecione el genero del familiar"); $("#sexo_f").focus(); } else if($("#parestesco_f").val()=="") { alert("Seleccione el parentesco del familiar"); $("#parestesco_f").focus(); } else if ($('#tipo_direccion').val()=="1") { 
			...
			else if($("#email_f").val().length==0){ alert("Seleccione los familiares del familiar"); $("#email_f").focus(); } else { valor = true; }
		} else { 
...
if($("#domicilio_f").val().length==0) { alert("Introduzca el domicilio del familiar"); $("#domicilio_f").focus(); } else if($("#estados_f").val()=="") {alert("Seleccione el estado al cual pertenece el familiar");$("#estados_f").focus(); } else if($("#municipios_f").val()=="") { alert("Selecione el municipio al cual pertenece el familiar"); $("#municipios_f").focus(); } else if($("#localidad_f").val().length==0) { alert("Seleccione la localidad a la cual pertenece el familiar"); $("#localidad_f").focus(); } else if($("#codigo_postal_f").val().length==0){ alert("Seleccione el Codigo Postal al cual pertenece el familiar");$("#codigo_postal_f").focus(); }else if($("#ine_f").val().length==0){ alert("Introduzca la clave INE,IFE del familiar"); $("#ine_f").focus(); } else if($("#rfc_f").val().length==0) { alert("Introduzca el RFC del familiar"); $("#rfc_f").focus();} else if($("#telefono1_f").val().length==0) { alert("Introduzca el Telefono Celular del familiar"); $("#telefono1_f").focus(); } else if($("#telefono2_f").val().length==0){ alert("Introduzca el Telefono de Casa del familiar"); $("#telefono2_f").focus(); } else if($("#telefono3_f").val().length==0){alert("Introduzca el Telefono de Oficina del familiar"); $("#telefono3_f").focus(); } else if($("#email_f").val().length==0){ alert("Seleccione los familiares del familiar"); $("#email_f").focus(); } else { valor = true; }
 ...
 return valor;
    ...
    $.fn.activeCalendary = function ( options){
		...
		$(options).val('');///PARA QUITAR LA FECHA POR DEFAULT***************************************************
	...
	$.fn.guardar_relacion = function()
	{
		...
		{
			alertify.error("Debe de seleccionar un cliente para relacionar");
			$('#nombre_relacion').focus();
		}
		else
			if ($('#relaciones').val()=="")
			{
				alertify.error("Debe seleccionar la relación que se tiene con dicho cliente");
				$('#relaciones').focus();
			}
			else
				if ($('#parentesco').val()=="")
				{
					alertify.error("Debe seleccionar el parentesco que se tiene con la persona seleccionada");
					$('#parentesco').focus();
				}
				else
				{
					if($('#relaciones').val()=="2" && ($('#aval_hist').val()=='' || $('#aval_capacidad').val()=='' || $('#aval_solvencia').val()==''))
						alertify.error("Debe llenar todos los campos obligatorios del aval");

					else if($('#relaciones').val()=="1" && $('#porcentaje').val()=="")// es beneficiario
						alertify.error("Debe colocar un porcentaje");

					else{
						var aval_hist_desc = $("#aval_hist option:selected").text();
						var aval_capacidad_desc = $("#aval_capacidad option:selected").text();
						var aval_solvencia_desc = $("#aval_solvencia option:selected").text();


						$.ajax({
							type: "post",
							url:"../php/safe_general.php",
							data:"module=add_relacion_to_clients&idkey_cliente="+$('#idkey_cliente').val()+"&addidkey_cliente="+$('#addidkey_cliente').val()+
							"&relaciones="+$('#relaciones').val()+"&parentesco="+$('#parentesco').val()+"&nombre="+$('#nombre_relacion').val()+
							"&aval_hist="+$('#aval_hist').val()+"&aval_capacidad="+$('#aval_capacidad').val()+"&aval_solvencia="+$('#aval_solvencia').val()+
							"&porcentaje="+$('#porcentaje').val()+"&aval_hist_desc="+aval_hist_desc+"&aval_capacidad_desc="+aval_capacidad_desc+
							"&aval_solvencia_desc="+aval_solvencia_desc+"&idkey_cliente_rel="+$('#addidkey_cliente').val(),
							success:function(resultdado){ 
								alertify.success("Relación guardada exitosamente");
								if($('#relaciones').val()=="1")// es beneficiario
									alertify.success("Recuerda verificar que el % total de los beneficiarios debe sumar 100%");
								$.fn.agregar_relaciones_cancel();
							}
						});
					}
				}
   ...
   $.fn.generar_factores = function()
	...
	$.fn.guardar_contacto = function()
	    {
	     var regex = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;//para validar el mail
  
		if (document.getElementById('idkey_cliente').value=="0")
		    {
			alert("Error no se puede agregar la informacioón de contacto hasta que guarde los datos Generales y Domicilios");
			document.getElementById("nombre").focus();
		    }
		else
		    if (document.getElementById("contacto_descripcion").value.length==0)
			{
			    alertify.error("Proporcione la descripción de la información de contacto");
			    document.getElementById("contacto_descripcion").focus();
			}
		    else
			if (document.getElementById("contacto_telefono").value.length==0)
			    {
				alertify.error("Proporcione el teléfono de contacto del nuevo cliente");
				document.getElementById("contacto_telefono").focus();
			    }
			else
			    if (document.getElementById("contacto_email").value.length>0 && !regex.test(document.getElementById("contacto_email").value))
				{
				    alertify.error("El email no tiene un formato correcto");
					document.getElementById("contacto_email").focus();
				}
			    else
					if (document.getElementById("contacto_prioridad").selectedIndex==0){
						alertify.error("Debe proporcionar la prioridad de contacto del nuevo cliente");
						...
						$.ajax({
						    ...
						    success:function(resultdado){ 
						    	$.fn.agregar_contactos_cancel(); 
						    	alertify.success("Contacto guardado exitosamente");
						    }
						    });
				    }
				}
		...
		$.fn.guardar_egreso = function()
			{
				...
				if (document.getElementById("tipo_egreso").selectedIndex==0)
					...
					if (document.getElementById("monto_egreso").value.length==0)
						...
						success:function(resultdado){ 
							alertify.success("Egreso guardado exitosamente");
							$.fn.agregar_egreso_cancel(); }
						});
					    }
					};   
	}( jQuery ));
...
// Guardar garantias Muebles By Moshe Ramz
	...
	{
	    if ($('#tipo_pago').val().length==0)
	    {
		alert("Debe indicar tipo de pago");
		$('#tipo_pago').focus();
	    }
	    else   
	    {
	    ...
	    type: "post",
	    url:"../php/safe_general.php",
	    data:{module:"registrar_pago",
		idkey_credito:$('#idkey_credito').val(),
		tipo_pago:$('#tipo_pago').val(),
		referencia:$('#referencia').val(),
		fecha_valor:$('#fecha_valor').val(),
		pago_valor:$('#pago_valor').val(),
		fila_id:$('#fila_id').val()
		},
	    success:function(data){ 
		location.href="../perfil-promotor.php"; 
		alertify.notify("Pago registrado");
		}
		});
	    }
	}
	...
	function guardar_factores()
	{
		...
		alertify.alert("Debe selecionar una opcion en Referencias", function(){
			alertify.message('OK');
			});
			$('#referencias').focus();
			}
			else
				if ($('#veracidad').val().length==''){
				alertify.alert("Debe selecionar una opcion en Veracidad", function(){
					alertify.message('OK');
					});
					$('#veracidad').focus();
					}
					else
						if ($('#garantia_liquida').val().length==''){
						alertify.alert("Debe selecionar una opcion en Garantia Liquida", function(){
							alertify.message('OK');
							});
							$('#garantia_liquida').focus();
							}
							else
								if ($('#conocimiento').val().length==''){
								alertify.alert("Debe selecionar una opcion en Conocimiento", function(){
									alertify.message('OK');
									});
									$('#conocimiento').focus();
									}
	...
	function guardar_grupo()
	{
		if ($('#nombre_grupo').val()=="")
		{
			alertify.alert("Debe indicar un nombre de grupo", function(){
			alertify.message('OK');
			});
			$('#nombre_grupo').focus();
		}
			...
			success:function(data){
					nexturl = "form-creargrupo.php?idkey_cliente="+$.trim(data);
					//alert(nexturl);
	...
	// actualizacion integrantes del grupo
	...
	//Actualiza los porcentajes de cada socio en un grupo definido
	function guardar_porcentajes()
	{	
	    ...
	    type: "post",
	    url:"../php/safe_general.php",
	    data:{module:"guardar_porcentajes",
		idkey_cliente:$('#idkey_cliente').val(),
		folio:$('#folio').val(),
		post_settings
		},
	    success:function(data){ 
		//alert(data);
		location.href="form-creargrupo.php?idkey_cliente="+idkey_cliente+"&porcentaje=true";
		}
		});
	};
	...
	{
		alertify.alert("Debe seleccionar un producto", function(){
			alertify.message('OK');
			});
		$('#tipo_producto').focus();
	}
	else
		if($('#monto_credito').val().length==0)
		{
			alertify.alert("Debe indicar el monto solicitado", function(){
				alertify.message('OK');
				});
			$('#monto_credito').focus();
		}
		else
			if($('#plazo_meses').val().length==0)
			{
				alertify.alert("Debe indicar el plazo en meses", function(){
					alertify.message('OK');
					});
				$('#plazo_meses').focus();
			}
			else
				if($('#frecuencia_pago').val().length==0)
				{
					alertify.alert("Debe indicar la frecuencia de pagos", function(){
						alertify.message('OK');
						});
					$('#frecuencia_pago').focus();
				}
				else
					if($('#fecha_pago1').val().length==0)
					{
						alertify.alert("Debe indicar fecha de primer pago", function(){
							alertify.message('OK');
							});
						$('#fecha_pago1').focus();
					}
					...
					success:function(data){ 
						...
						alertify.success("Solicitud enviada correctamente");
