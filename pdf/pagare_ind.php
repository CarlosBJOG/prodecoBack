...
//obtenemos direccion y ciudad del cliente

$query = "SELECT nombre_mpio, nombre_edo, nombre_loc, nombre_cp, domicilio FROM view_direcciones WHERE idkey_cliente = $ClienteNo and prioridad=1";
$data2 = $oconns->getRows($query);
if ($oconns->numberRows>0){
  $estado =  utf8_decode(mb_strtoupper($data2[0]["nombre_edo"]));
  $municipio = utf8_decode(mb_strtoupper($data2[0]["nombre_mpio"]));
  $localidad = utf8_decode(mb_strtoupper($data2[0]["nombre_loc"]));
  $cp = utf8_decode(mb_strtoupper($data2[0]["nombre_cp"]));
  $domicilio = utf8_decode(mb_strtoupper($data2[0]["domicilio"]));
...
for($k=1; $k<=$rows_per_page; $k++)
{
  ...
  $date = date('d-m-Y');
  $pdf->Cell(110, 5, utf8_decode('PAGARÉ'), 0, 0, 'C');
  $pdf->SetFont('Arial','',5);
  $pdf->Cell(10,5,"BARRA DE NAVIDAD OAXACA,Fecha: $date", 0, 1, 'C');
  $pdf->Cell(176,3,utf8_decode("Núm. de Cliente:$ClienteNo Pagaré Núm. :"), 0, 1, 'R');
  $pdf->SetFont('Arial','',6);
