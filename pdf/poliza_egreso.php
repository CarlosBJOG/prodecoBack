...
$data = $oconns->getRows("select pe.fecha, pe.no_poliza, pe.concepto, pe.monto, serie, tipo from poliza_egreso pe where idkey='$idkey_poliza_egreso'");
...
$pdf->AddPage();
 ...
 $pdf->SetFont('Arial','',8);
 $pdf->Cell(10,4,"", 0, 0, 'L');
 $pdf->Cell(90,4,utf8_decode("NO. PÓLIZA: ".$data[0]["no_poliza"]), 0, 0, 'L');
 $pdf->Cell(50,4,"SERIE: ".$data[0]["serie"]." , TIPO: ".$data[0]["tipo"], 0,1, 'L');

 $pdf->Cell(10,4,"", 0, 0, 'L');
 $pdf->Cell(90,4,utf8_decode("FECHA PÓLIZA: ".$data[0]["fecha"]), 0, 0, 'L');
 $pdf->Cell(50,4,utf8_decode("FECHA IMPRESIÓN: ".date('Y-m-d H:i:s')), 0, 1, 'L');
