...
if($idkey_credito === '0'){
    ...
    $datos = $oconns->getRows("SELECT gc.idkey, gc.idkey_credito, gc.fecha_registro, gc.monto, gc.folio_operacion, gc.banco, vc.nombre, vc.nombre_producto FROM gastos_cobranza AS gc INNER JOIN view_creditos as vc ON vc.idkey_credito = gc.idkey_credito WHERE gc.idkey = '$idkey' ORDER BY gc.fecha_registro DESC LIMIT 1 ");
}else{
    $datos = $oconns->getRows("SELECT gc.idkey, gc.idkey_credito, gc.fecha_registro, gc.monto, gc.folio_operacion, gc.banco, vc.nombre, vc.nombre_producto FROM gastos_cobranza AS gc INNER JOIN view_creditos as vc ON vc.idkey_credito = gc.idkey_credito WHERE gc.idkey_credito = '$idkey_credito' ORDER BY gc.fecha_registro DESC LIMIT 1 ");
}
...
if(count($datos) > 0){
    ...
    $folio_operacion = mb_strtoupper($datos[0]['folio_operacion']);
