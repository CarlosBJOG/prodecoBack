...
// }
$datos = $oconns->getRows("SELECT * FROM seguros_deposito WHERE idkey = '$idkey'");
if(count($datos) > 0){
    $id =mb_strtoupper($datos[0]['idkey']);
    $fecha =mb_strtoupper($datos[0]['fecha_registro']);
    $monto = mb_strtoupper($datos[0]['monto']);
    $folio = mb_strtoupper($datos[0]['folio_operacion']);
...
$pdf->Cell(50, 3,"Id de Pago: $id", 0, 1, 'L', 0);
$pdf->Cell(50, 3,utf8_decode("Fecha: $fecha"), 0, 1, 'L', 0);

$pdf->Ln(3);

$pdf->Cell(50, 3,utf8_decode("Seguros Depósito"), 0, 1, 'L', 0);

$pdf->Ln(3);
if($idkeyFolio == '1'|| $idkeyFolio ==1){
